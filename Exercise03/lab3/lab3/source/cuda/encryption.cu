...
__global__ void find_hash(const std::uint64_t searched_hash, const std::uint64_t *const hashes, unsigned int *const indices, const unsigned int length, unsigned int *const ptr) {
