...
public:
    ...
    static block_type encrypt_block(const block_type &currBlock, const block_type &prevBlock)
    {
        block_type encryptedBlock;
        row_type currRow;
        row_type prevRow;
        row_type combinedRow;
        row_type result;
        for (int y = 0; y < 3; ++y)
        {
            for (int x = 0; x < 3; ++x)
            {
                currRow = currBlock[x][y];
                prevRow = prevBlock[x][y];
                combinedRow = combine_rows(currRow, prevRow);
                if (x == 0 && y == 0)
                { // for the row at [0][0]
                    result = combinedRow;
                }
                ...
                { // for rows at [0][y]
                    result = combine_rows(combinedRow, encryptedBlock[2][y - 1]);
                }
                else
                { // for other rows
                    result = combine_rows(combinedRow, encryptedBlock[x - 1][y]);
                }
                // store the encrypted row in the encrypted block
                encryptedBlock[x][y] = result;
            }
        }
